...
describe('AwesomeLibrary', () => {
    ...
    describe('String Utilities', () => {
        test('truncateString shortens strings correctly', () => {
            ...
            expect(AwesomeLibrary.truncateString('Short string', 50)).toBe('Short string');
        });
        test('isValidEmail validates email addresses', () => {
            // Valid email addresses
            expect(AwesomeLibrary.isValidEmail('test@example.com')).toBe(true);
            expect(AwesomeLibrary.isValidEmail('user.name+tag@example.co.uk')).toBe(true);
        ...
        test('toSlug converts string to slug', () => {
            ...
            expect(AwesomeLibrary.toSlug('Special @ Characters!!!')).toBe('special-characters');
        });
    });
    ...
    describe('Array Utilities', () => {
        ...
        test('removeDuplicates eliminates duplicate values', () => {
            expect(AwesomeLibrary.removeDuplicates([1, 2, 2, 3, 4, 4, 5])).toEqual([1, 2, 3, 4, 5]);
            expect(AwesomeLibrary.removeDuplicates(['a', 'b', 'b', 'c'])).toEqual(['a', 'b', 'c']);
        });
    });
