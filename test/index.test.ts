...
describe('AwesomeLibrary', () => {
  ...
  describe('Number Utilities', () => {
    ...
    test('formatNumberToLocaleString formats numbers correctly', () => {
      expect(AwesomeLibrary.formatNumberToLocaleString(1000)).toBe('1,000');
      expect(AwesomeLibrary.formatNumberToLocaleString(1000.123)).toBe('1,000.123');
    })
  ...
  describe('String Utilities', () => {
    ...
    test('generateRandomString generates random string of specified length', () => {
      const length = 10;
      const randomString = AwesomeLibrary.generateRandomString(length);
      expect(randomString.length).toBe(length);
    });
    test('isAlphaNumeric checks if a string is alphanumeric', () => {
      expect(AwesomeLibrary.isAlphaNumeric('abc123')).toBe(true);
      expect(AwesomeLibrary.isAlphaNumeric('abc 123')).toBe(false);
    });

    test('countWords returns the number of words in a string', () => {
      expect(AwesomeLibrary.countWords('Hello, World!')).toBe(2);
      expect(AwesomeLibrary.countWords('The quick brown fox')).toBe(4);
    });

    test('toKebabCase converts string to kebab-case', () => {
      expect(AwesomeLibrary.toKebabCase('Hello, World!')).toBe('hello-world');
      expect(AwesomeLibrary.toKebabCase('Test String')).toBe('test-string');

    });

    test('toCamelCase converts string to camelCase', () => {
      expect(AwesomeLibrary.toCamelCase('Test String')).toBe('testString');
    });
  ...
  describe('Helper Utility', () => {
    test('memoize should cache function results', () => {
      ...
      const memoized = AwesomeLibrary.memoize(callback);
      ...
      // The function should only be called once for each unique argument
      expect(callback).toHaveBeenCalledTimes(2);
    });

    test('generateRandomColor should generate random hex color', () => {
      const color = AwesomeLibrary.generateRandomColor();
      expect(color).toMatch(/^#[0-9a-f]{6}$/i);
    });

    test('rgbToHex should convert RGB values to hex color', () => {
      expect(AwesomeLibrary.rgbToHex(255, 0, 0)).toBe('#FF0000');
      expect(AwesomeLibrary.rgbToHex(0, 255, 0)).toBe('#00FF00');
      expect(AwesomeLibrary.rgbToHex(0, 0, 255)).toBe('#0000FF');
    });
  });
});
